...
ig.Controller = ig.Class.extend({
	...
	generateCreatures: function() {
		...
		for (var i = 0; i < this.slots.length; i++) {
			...
			if (randInt === 1 && !targetCreatureSpawned) {
	...
	rollRandomCreature: function() {
		var randInt = this.randomFromTo(0, this.creatureLibraryArr.length - 1);
		var isDuplicate = this.creatureExists(this.creatureLibraryArr[randInt]);
		if (isDuplicate || this.creatureLibraryArr[randInt].kind === this.targetCreature.kind) {
			console.log('rerolling');
			return this.rollRandomCreature();
		}
		else {
			return this.creatureLibraryArr[randInt];
		}
	},
